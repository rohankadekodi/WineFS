...
load_parameters=' --write_buffer_size=67108864 --open_files=32768 --level0_slowdown_writes_trigger=80 --level0_stop_writes_trigger=100 --mmap_read=true --mmap_write=true --allow_concurrent_memtable_write=true --disable_wal=false --num_levels=7 --memtable_use_huge_page=true --target_file_size_base=67108864 --max_bytes_for_level_base=17179869184 --max_bytes_for_level_multiplier=10 --max_background_compactions=24 --delayed_write_rate=268435456 --subcompactions=24 --hard_pending_compaction_bytes_limit=274877906944 --soft_pending_compaction_bytes_limit=137438953472 --level0_file_num_compaction_trigger=64 --max_background_flushes=2 --max_write_buffer_number=4'
...
load_workload()
{
    ...
    date
    if [ "$fs" == "splitfs" ]; then
        numactl --cpubind=0 $splitfsDir/run_boost.sh -p $splitfsDir -t nvp_nvp.tree ./db_bench --use_existing_db=0 --benchmarks=ycsb,stats,levelstats,sstables --db=$databaseDir --compression_type=none --threads=${num_of_threads} $load_parameters 2>&1 | tee $resultDir/Run${runId}_${num_of_threads}
    else
        numactl --cpubind=0 --membind=0 ./db_bench --use_existing_db=0 --benchmarks=ycsb,stats,levelstats,sstables --db=$databaseDir --compression_type=none --threads=${num_of_threads} $load_parameters 2>&1 | tee $resultDir/Run${runId}
    fi
    date
...
run_workload()
{
    ...
    date
    if [ "$fs" == "splitfs" ]; then
        numactl --cpubind=0 --membind=0 $splitfsDir/run_boost.sh -p $splitfsDir -t nvp_nvp.tree ./db_bench --use_existing_db=1 --benchmarks=ycsb,stats,levelstats,sstables --db=$databaseDir --compression_type=none --threads=${num_of_threads} $run_parameters 2>&1 | tee $resultDir/Run${runId}_${num_of_threads}
    else
        numactl --cpubind=0 --membind=0 ./db_bench --use_existing_db=1 --benchmarks=ycsb,stats,levelstats,sstables --db=$databaseDir --compression_type=none --threads=${num_of_threads} $run_parameters 2>&1 | tee $resultDir/Run$runId
    fi
    date
